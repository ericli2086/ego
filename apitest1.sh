...
run_case "批量创建" \
  "curl -s -X POST \"$API/$DB/$TABLE\" -H \"Content-Type: application/json\" -d '[
    {\"username\":\"alice$TIME\",\"email\":\"alice$TIME@example.com\"},
    {\"username\":\"bob$TIME\",\"email\":\"bob$TIME@example.com\"}
  ]'" \
  "201"
...
# 20. icontains 查询
run_case "username__icontains=bob" \
  "curl -s \"$API/$DB/$TABLE?username__icontains=bob&page=1&page_size=10&fields=$KEY,username\"" \
  "200" \
  ".data | length > 0" "true"
